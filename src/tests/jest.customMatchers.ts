...
function toBeAtom(container: HTMLElement): CustomMatcherResult {
...
function toBeMolecule(container: HTMLElement): CustomMatcherResult {
  const keys = getRoleKeys(container);
  if (!(keys.length >= 2)) {
...
function toBeOrganism(container: HTMLElement): CustomMatcherResult {
  const keys = getRoleKeys(container);
  if (!(keys.length >= 2)) {
...
function toBeTemplate(container: HTMLElement): CustomMatcherResult {
  const keys = getRoleKeys(container);
  if (!includeMainRole(keys)) {
